...
class TrackController(private val trackService: TrackService) {
    ...
    fun getRoute(@PathVariable startStopId: Int,
                 ...
                 @PathVariable timestamp: Long): List<TrackDto> {
        ...
        val result = trackService.dijkstra(startId, endId, timestamp, false)
                ?: throw ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Dijkstra failed to find a solution!")
        return List(result.lines.size) { i -> TrackDto(result.lines[i], result.waitTime[i], if (result.stops[i] == null) null else result.stops[i]!!.mapIndexed { j, x -> StopTimeDto(x!!, result.names[i]!![j])}, result.from[i], result.to[i], result.time[i])}
    }
}